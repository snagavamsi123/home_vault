...
// class OldRecentFiles extends StatefulWidget {
//   const OldRecentFiles({Key? key}) : super(key: key);

//   @override
//   _OldRecentFilesState createState() => _OldRecentFilesState();
// }

// class OldRecentFile {
//   final String? icon, title, date, size, file_url;

//   OldRecentFile({this.icon, this.title, this.date, this.size, this.file_url});
// }

// class _OldRecentFilesState extends State<OldRecentFiles> {
//   List<OldRecentFile> OldRecentFiles = [];
//   bool isLoading = true;
...
//     final response =
//         await http.get(Uri.parse('http://10.0.2.2:9001/api/fetch_recent_data'));
//     if (response.statusCode == 200) {
//       final List<dynamic> responseData = json.decode(response.body);
//       print('Response Data: $responseData'); // Enhanced logging
//       setState(() {
//         OldRecentFiles = responseData
//             .map((data) => OldRecentFile(
//                   title: data['filename'],
...
//                     ],
//                     rows: OldRecentFiles.map((file) {
//                       return OldRecentFileDataRow(file);
//                     }).toList(),
//                   ),
//           ),
//         ],
...
// DataRow OldRecentFileDataRow(OldRecentFile fileInfo) {
//   return  DataRow(
...
//                     ],
//                     rows: OldRecentFiles.map((file) {
//                       return OldRecentFileDataRow(context, file);
//                     }).toList(),
//                   ),
//           ),
//         ],
...
// DataRow OldRecentFileDataRow(BuildContext context, OldRecentFile fileInfo) {
//   return DataRow(
...
class _OldRecentFilesState extends State<OldRecentFiles> {
  ...
  void initState() {
    ...
    channel.stream.listen((message) {
      // Handle message
      setState(() {
        OldRecentFiles = (json.decode(message) as List)
            .map((data) => OldRecentFile(
                  title: data['filename'],
                  icon: _getIcon(data['file_type']),
                  date: data['created_at'],
        ...
        print('recentFielesss $OldRecentFiles');
        isLoading = false;
      });
    });
  }
